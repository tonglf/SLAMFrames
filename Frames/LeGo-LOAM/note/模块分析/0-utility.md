...
该文件内包含了一些全局的变量，一些变量与后面模块紧密结合，明白一些变量的含义有助于理解后续模块的内容。
...
// VLP-16
extern const int N_SCAN = 16;		// 16 线
extern const int Horizon_SCAN = 1800;	// 一圈有多少点（360°）360/0.2=1800
extern const float ang_res_x = 0.2;		// 水平角度
extern const float ang_res_y = 2.0;		// 垂直角度
extern const float ang_bottom = 15.0+0.1;	// 最低线（第0线）对应的角度
extern const int groundScanInd = 7;		// 能够扫描到地面的扫描线，例：第10线扫描到的不是地面
...
extern const int segmentValidPointNum = 5;		// 有效点
extern const int segmentValidLineNum = 3;		// 线特征的有效点
extern const float segmentAlphaX = ang_res_x / 180.0 * M_PI;	// 弧度值
